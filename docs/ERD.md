...
Hackaform's final capstone data model combines persistent relational data with authenticated ownership. A user can organize events and attend other users' events. Every event can have an ordered agenda, many attendee bookings, and one optional private attendee circle.
...
| `EventCircle` | Stores the private pre-event coordination invite and welcome note | Belongs to one event; managed by its organizer and readable only by that organizer or confirmed attendees |
...
- `event_circles.event_id` is unique, so an event can have at most one attendee circle; deleting the event deletes its circle.
...
- Circle invite URLs must use HTTPS on the exact `chat.whatsapp.com` host. Public event responses never contain them.
...
| EventCircle | `POST /api/events/:id/circle` | `GET /api/events/:id/circle` | `PATCH /api/events/:id/circle` | `DELETE /api/events/:id/circle` |

Protected requests send `Authorization: Bearer <access_token>`. Circle reads additionally require event ownership or a confirmed booking. This contract lets the React client use one owned API instead of the public feeds used in Phase 1.
